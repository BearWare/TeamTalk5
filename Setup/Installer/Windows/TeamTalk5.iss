...
en.InstallVCRedist=Install Visual C++ Redistributable (Recommended)
en.AdditionalSoftware=Additional software:
fa.Client=برنامۀ تیم‌تاک5
fa.Full=برنامۀ تیم‌تاک5 همراه با سِروِر
fa.InstallVCRedist=نصب Visual C++ Redistributable (توصیه می‌شود)
fa.AdditionalSoftware=نرم‌افزار اضافی:
fr.Client=Client TeamTalk 5
fr.Full=Client et Serveur TeamTalk 5
fr.InstallVCRedist=Installer Visual C++ Redistributable (Recommandé)
fr.AdditionalSoftware=Logiciels supplémentaires:
ko.Client=TeamTalk 5 클라이언트
ko.Full=TeamTalk 5 클라이언트 및 서버
ko.InstallVCRedist=Visual C++ 재배포 가능 패키지 설치 (권장)
ko.AdditionalSoftware=추가 소프트웨어:
zh_CN.Client=TeamTalk 5 客户端
zh_CN.Full=TeamTalk 5 客户端 & 服务器
zh_CN.InstallVCRedist=安装 Visual C++ 可再发行组件包 (推荐)
zh_CN.AdditionalSoftware=附加软件:

[Tasks]
Name: "desktopicon"; Description: "{cm:CreateDesktopIcon}"; GroupDescription: "{cm:AdditionalIcons}"; Flags: unchecked
Name: "quicklaunchicon"; Description: "{cm:CreateQuickLaunchIcon}"; GroupDescription: "{cm:AdditionalIcons}"; Flags: unchecked
Name: "vcredist"; Description: "{cm:InstallVCRedist}"; GroupDescription: "{cm:AdditionalSoftware}"; Components: client
...
[Run]
Filename: "{tmp}\vc_redist.x64.exe"; Parameters: "/install /quiet /norestart"; Tasks: vcredist; Check: Is64BitInstallMode;
...
function URLDownloadToFile(pCaller: Integer; szURL: AnsiString; szFileName: AnsiString; dwReserved: Integer; lpfnCB: Integer): Integer;
...
begin
  ...
  if not WizardIsTaskSelected('vcredist') then
    Exit;

  VCRedistPath := ExpandConstant('{tmp}\vc_redist.x64.exe');

  try
    hres := URLDownloadToFile(0, VCREDIST_URL, VCRedistPath, 0, 0);
    if hres <> 0 then
    begin
      MsgBox('Failed to download Visual C++ Redistributable. Error code: ' + IntToStr(hres), mbError, MB_OK);
      Result := False;
    end;
  except
    MsgBox('An error occurred while downloading Visual C++ Redistributable.', mbError, MB_OK);
    Result := False;
  end;
end;
...
begin
  Result := '';
  if WizardIsTaskSelected('vcredist') and Is64BitInstallMode then
  begin
    if not DownloadVCRedist() then
      Result := 'Failed to download Visual C++ Redistributable.';
  end;
end;
