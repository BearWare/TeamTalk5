        ...
        if(TT_GetFlags(ttInst) & CLIENT_AUTHORIZED)
        {
            ...
            if(idle_time != 0)
            {
                ...
                if (isComputerIdle(idle_time) && (m_statusmode & STATUSMODE_MODE) == STATUSMODE_AVAILABLE)
                {
                    ...
                    TT_DoChangeStatus(ttInst, m_statusmode, _W(statusmsg));
                    m_idled_out = true;
                    if (isMyselfTalking() == TRUE && ttSettings->value(SETTINGS_GENERAL_INACTIVITY_DISABLE_VOICEACT, SETTINGS_GENERAL_INACTIVITY_DISABLE_VOICEACT_DEFAULT).toBool() == true)
